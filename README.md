...
4. 名前  
   ...
   また、波括弧 { } で囲うことで、区切り記号や演算子記号を含む文字列も、  
   名前として使用できます ( {a-b-c}  等)。  
...
7. 文字列リテラル  
   ...
   ( y = "aaa" %% "bbb"  等)  
   文字列リテラル内で ダブルクォートの文字自身を表したい場合は、\\ でエスケープ  
   してください(\\")。  また、\\ 自身を表したい場合は、\\\\ と記述してください。

8. 制御命令(機能なし)  
   現状、if, for, while, switch, goto 等の制御命令は使用できません。  
   また、関数を独自に定義するような命令も未対応です。

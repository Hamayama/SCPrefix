...
4. 名前  
   ...
   波括弧内では、区切り記号や演算子の記号も通常の文字として使えます。  
   ( {-100}  {1+2i}  等)  
   波括弧内で 波括弧の文字自身を表すときは、\\ でエスケープします。  
   また、\\ 自身を表すときは、\\\\ と記述します。  
   波括弧の囲みは、変換時に外されて中身だけが出力されます。  
   ( {-100} → -100  等)  
   
   また、ダブルクォート(")で囲うことで、文字列リテラルを表します。  
   ダブルクォート内では、区切り記号や演算子の記号も通常の文字として使えます。  
   ( "a b c"  "==12345=="  等)  
   ダブルクォート内で ダブルクォートの文字自身を表すときは、\\ でエスケープします。  
   また、\\ 自身を表すときは、\\\\ と記述します。  
   現状、文字列リテラルも名前として扱われます。  
   また、文字列結合の演算子は + ではなく %% となっています。
